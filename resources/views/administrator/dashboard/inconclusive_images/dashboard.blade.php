        ...
        <section class="table-responsive mt-2">
            ...
            <table class="table table-hover mt-2">
                ...
                @foreach($images as $image)
                    <tr>
                        <td class="text-center">{{ ImageUpload::where('filename_post_iva', $image)->first()->id }}</td>
                        <td class="text-center">
                            <div class="row">
                                <div class="col">
                                    <div class="d-flex justify-content-center">
                                        @if(empty(ImageUpload::where('filename_post_iva', $image)->first()->filename_pre_iva))
                                            <img src="{{ asset('assets/images/no-image.png') }}" height="72px">
                                        @else
                                            <img
                                                src="{{ url('files/images/iva/'.ImageUpload::where('filename_post_iva', $image)->first()->filename_pre_iva) }}"
                                                height="72px">
                                        @endif
                                    </div>
                                ...
                                <div class="col">
                                    <div class="d-flex justify-content-center">
                                        <img
                                            src="{{ url('files/images/iva/'.ImageUpload::where('filename_post_iva', $image)->first()->filename_post_iva) }}"
                                            height="72px">
                                    </div>
                        ...
                        <td>
                            <section>
                                @if(count(ImageLabel::where('filename', ImageUpload::where('filename_post_iva', $image)->first()->filename_post_iva)->get()) != 0)
                                    @if(count(ImageLabel::where('filename', ImageUpload::where('filename_post_iva', $image)->first()->filename_post_iva)->get()) == 1 && ImageLabel::where('filename', ImageUpload::where('filename_post_iva', $image)->first()->filename_post_iva)->first()->label == ImageUpload::IMAGE_NOT_LABELED_CODE)
                                        <span class="font-italic">Belum ada ekspertise</span>
                                    @else
                                        <ul>
                                            @foreach(ImageLabel::where('filename', ImageUpload::where('filename_post_iva', $image)->first()->filename_post_iva)->get() as $image)
